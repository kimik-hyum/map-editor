...
const eyebrowClassName =
  "mb-2.5 mt-0 text-[13px] font-extrabold uppercase text-[#0f766e]";
const leadClassName = "mt-[18px] max-w-[640px] text-lg leading-[1.7] text-[#506174]";
const sectionClassName = "grid gap-[22px] border-t border-[#d6e0e7] py-[34px]";
const sectionTitleClassName = "m-0 text-[28px] font-bold text-[#172033]";
const itemTitleClassName = "mb-2 mt-0 text-base font-bold text-[#172033]";
const itemDescriptionClassName = "m-0 leading-[1.6] text-[#5f6f82]";
    ...
    <main className="mx-auto max-w-[1120px] px-6 pb-16 pt-10 max-[560px]:px-4 max-[560px]:pb-12 max-[560px]:pt-7">
      <section className="grid min-h-[360px] grid-cols-[minmax(0,1fr)_minmax(320px,420px)] items-center gap-8 selection:bg-teal-100 selection:text-teal-950 max-[900px]:min-h-0 max-[900px]:grid-cols-1">
        <div>
          ...
          <h1 className="m-0 text-[clamp(40px,7vw,72px)] leading-[1.1] text-[#172033]">
            지도 도형 편집기 설계 메모
          </h1>
        ...
        >
          ...
          <span className="absolute left-[25%] top-[35%] z-[3] h-[18px] w-[18px] rounded-full border-[3px] border-white bg-[#172033]" />
          <span className="absolute bottom-[24%] right-[26%] z-[3] h-[18px] w-[18px] rounded-full border-[3px] border-white bg-[#172033]" />
        </div>
      </section>
      ...
      <section className={sectionClassName}>
        ...
        <div className="grid grid-cols-4 gap-3.5 max-[900px]:grid-cols-1">
          {stackItems.map((item) => (
            <article
              className="rounded-lg border border-[#d6e0e7] bg-white p-[18px]"
              key={item.name}
            >
      ...
      <section className={sectionClassName}>
        ...
        <div className="grid gap-2.5">
          {plannedPages.map((page) => (
            <Link
              className="grid grid-cols-[120px_minmax(0,1fr)] items-start gap-[18px] rounded-lg border border-[#d6e0e7] bg-white px-[18px] py-4 no-underline transition-[border-color,transform] duration-150 ease-in-out hover:-translate-y-px hover:border-[#9bcfc3] max-[560px]:grid-cols-1"
              key={page.path}
              to={page.path}
            >
              <code className="font-extrabold text-[#0f766e]">{page.path}</code>
              <div>
