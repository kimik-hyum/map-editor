...
// 이름과 +/- 버튼을 "두 행"으로 함께 보여줘 둘 다 노출되게 한다 — 이름 행(전체 표시, 길면 줄바꿈) +
// 버튼 행. 한 행에 욱여넣다 긴 이름이 잘려 사라지던 문제를 피한다. 이름이 없으면 버튼 행만 보인다.
...
export function GeometryOpMarker({
  ...
  return (
    <div
      className={`flex max-w-[200px] flex-col items-center gap-1 bg-white/95 shadow-lg ring-1 ring-slate-200 ${
        name ? "rounded-2xl px-2 py-1" : "rounded-full p-0.5"
      }`}
    >
      {name ? (
