...
경계 이름·작업 카드는 화면 안에서 상시 표시합니다. 줌 12 미만/12–14/15 이상에서 이름은 14/15/16px, 버튼 높이는 32/34/36px입니다. 화면 픽셀 기준 겹침과 개수 제한(24/48/72개)을 적용하고, 카드가 생략된 경계는 캔버스 이름을 유지합니다. 포인터·클릭은 밀집 후보를 우선 표시하는 보조 경로이며 키보드로 포커스한 카드는 우선 보존합니다. 카드 위치·투영 geometry·배치 캐시는 adapter가 소유하며, 표시만으로 원본 API나 scene 쓰기를 실행하지 않습니다.

경계의 주 버튼은 편집 가능한 폴리곤 단독 선택이면 `합치기`, 그 외에는 `추가`입니다. `빼기`는 실제 겹침이 있을 때만 활성화하며 조회 중에는 버튼을 비활성화합니다. React 작업 카드를 표시한 경계는 캔버스 이름을 감춰 중복 라벨을 방지합니다. 색상은 앱 brand(청록)·danger(장미색) 토큰으로 통일하고 아이콘만이 아니라 동작 문구와 native title을 함께 제공합니다.
