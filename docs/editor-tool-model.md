...
편집 도형끼리의 교집합은 선명한 보라(#8000ff) 배경과 흰 아이콘으로 추가/빼기와 구분합니다. 세 연산 버튼 모두 흰 아이콘과 배경의 대비를 4.5:1 이상 유지합니다. 실제 응답 종류가 행정동(adminDong)이면 참고 경계선을 검정 2.5px로 표시하고, 법정동·우편번호·줌 축소 시 시군구 대체 경계는 기존 청록 1.5px를 유지합니다.
