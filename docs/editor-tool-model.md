...
경계의 `+` 아이콘은 편집 가능한 폴리곤 단독 선택이면 합치기, 그 외에는 추가입니다. `−`는 실제 겹침이 있을 때만 활성화하며 조회 중에는 버튼을 비활성화합니다. 동작 문구는 화면에 반복하지 않고 aria-label·native title로 설명합니다. React 이름을 표시한 경계는 캔버스 이름을 감춰 중복 라벨을 방지합니다. 이름은 흰색 사각 배경 대신 얇은 글자 테두리로 지도와 구분합니다. 버튼은 옅은 청록색으로 통일하고 호버·포커스에서만 강조하며 빼기는 호버 시 danger 색을 사용합니다.
